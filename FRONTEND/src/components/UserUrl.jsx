...
const UserUrl = () => {
  ...
  const { data, isLoading, isError, error } = useQuery({
    ...
    onError: (err) => console.error("Error fetching URLs:", err)
  })
  ...
  if (!isAuthenticated) {
    return (
      <div className="text-center text-gray-500 my-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
        ...
        <p className="mt-1">Log in to see your shortened URLs.</p>
      </div>
    )
  }
  ...
  return (
    <div className="bg-white rounded-lg mt-5 shadow-md overflow-hidden">
      <h2 className="text-xl font-semibold p-4 border-b">Your URLs</h2>
              ...
              <tr key={url._id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4">
                  <div className="text-sm">
                    <a 
                      href={`https://url-shortner-backend-g0lg.onrender.com/${url.short_url}`} 
                      target="_blank" 
                      rel="noopener noreferrer"
                      className="text-blue-600 hover:text-blue-900 hover:underline"
                    >
                      {`https://url-shortner-backend-g0lg.onrender.com/${url.short_url}`}
                    </a>
                  </div>
                </td>
                <td className="px-6 py-4">
